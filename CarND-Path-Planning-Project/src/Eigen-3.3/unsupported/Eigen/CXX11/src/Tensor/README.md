...
duration.  Fixed sized tensors can provide very fast computations because all their
...
duration.  This is done by inserting a call to the ```eval()``` method of the
expression Operation.

    // The previous example could have been written:
    Tensor<float, 3> result = ((t1 + t2) * 0.2f).exp();

    // If you want to compute (t1 + t2) once ahead of duration you can write:
...
See ```struct UniformRandomGenerator``` in TensorFunctors.h for an example.
    ...
    struct MyRandomGenerator {
      ...
      // Same as above but generates several numbers at a duration.
...
4th-Nth dimension: duration (for video) or batch (for bulk processing).

*) RowMajor (reverse order of ColMajor)
1st-Nth dimension: duration (for video) or batch (for bulk processing).
